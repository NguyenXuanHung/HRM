<%@ Page Language="C#" AutoEventWireup="true" Inherits="Web.HJM.Modules.ProfileHuman.Management.RewardDisciplineManagement" Codebehind="RewardDisciplineManagement.aspx.cs" %>
...
<head runat="server">
    ...
    <script type="text/javascript" src="../../Home/chart/js/jquery.min.js"></script>
    <style type="text/css">
        div#grp_KhenThuongKyLuat .x-grid3-cell-inner, .x-grid3-hd-inner {
    ...
    <script type="text/javascript">
        ...
        var CheckInputKTKLHangLoat = function (el) {
            if (cbbHinhThucHangLoat.getValue().trim() == '') {
                if (hdfTypePage.getValue() == "KhenThuong") {
                    alert('Bạn chưa chọn hình thức khen thưởng');
                } else {
                    alert('Bạn chưa chọn hình thức kỷ luật');
                }
                cbHinhThucHangLoat.focus();
                return false;
            }
            if (cbbLyDoHangLoat.getValue().trim() == '' && cbbLyDoHangLoat.getRawValue().trim() == '') {
                if (hdfTypePage.getValue() == "KhenThuong") {
                    alert('Bạn chưa chọn lý do khen thưởng');
                } else {
                    alert('Bạn chưa chọn lý do kỷ luật');
                }
                cbLyDoHangLoat.focus();
                return false;
            }
            if (hdfCapKhenThuongKyLuatHangLoat.getValue() == '') {
                if (hdfTypePage.getValue() == "KhenThuong") {
                    alert('Bạn chưa chọn cấp khen thưởng');
                } else {
                    alert('Bạn chưa chọn cấp kỷ luật');
                }
                cbbCapKhenThuongKyLuatHangLoat.focus();
                return false;
            }
...
<body>
    <form id="form1" runat="server">
        <div>
            ...
            <ext:Store ID="StoreCapKhenThuongKyLuat" AutoSave="false" ShowWarningOnFailure="false"
                SkipIdForNewRecords="false" RefreshAfterSaving="None" AutoLoad="false" OnRefreshData="StoreCapKhenThuongKyLuat_OnRefreshData"
                runat="server">
                <Reader>
                    <ext:JsonReader IDProperty="Id">
                        <Fields>
                                ...
                                <TopBar>
                                    <ext:Toolbar runat="server" ID="toolbarFn">
                                        <Items>
                                            ...
                                            <ext:ToolbarSpacer Width="5" />
                                            <ext:DisplayField runat="server" Text="Ngày quyết định từ ngày: " />
                                            <ext:ToolbarSpacer Width="5" />
                                            <ext:DateField ID="dfNgayBatDau" runat="server" Width="120"
                                            ...
                                            <ext:DisplayField runat="server" Text="đến ngày: " />
                                            <ext:ToolbarSpacer Width="5" />
                                            <ext:DateField ID="dfNgayKetThuc" runat="server"
                                                AnchorHorizontal="100%" MaskRe="/[0-9\/]/" Format="d/M/yyyy" Vtype="daterange" Width="120"
                                                Regex="/^(3[0-1]|[0-2]?[0-9])\/(1[0-2]|0?[0-9])\/[0-9]{4}$/" RegexText="Định dạng ngày hạn nộp hồ sơ không đúng">
                                                <CustomConfig>
                                ...
                                <Store>
                                    <ext:Store runat="server" ID="stKhenThuongKyLuat">
                                        ...
                                        <BaseParams>
                                            ...
                                            <ext:Parameter Name="TimeSheetHandlerTypePage" Value="hdfTimeSheetHandlerTypePage.getValue()" Mode="Raw" />
                                                    ...
                                                    <ext:HeaderColumn AutoWidthElement="true">
                                                        <Component>
                                                            <ext:ComboBox runat="server" ID="cbbCapKTKLFilter" DisplayField="Name"
                                                                ValueField="Id" AnchorHorizontal="100%" ListWidth="200" Editable="false" ItemSelector="div.list-item" StoreID="StoreCapKhenThuongKyLuat"
            ...
            <ext:Window ID="wdKhenThuong" AutoHeight="true" Width="550" runat="server" Padding="6"
                ...
                <Items>
                    ...
                    <ext:ComboBox runat="server" ID="cbxCapKhenThuong" DisplayField="Name" FieldLabel="Cấp khen thưởng - kỷ luật" TabIndex="6"
                        ValueField="Id" AnchorHorizontal="99%" Editable="false" ItemSelector="div.list-item" StoreID="StoreCapKhenThuongKyLuat"
            ...
            <ext:Window runat="server" ID="wdKhenThuongKyLuatHangLoat" Constrain="true" Modal="true"
                ...
                <Items>
                    <ext:Container runat="server" ID="ctn30" Layout="FormLayout" AutoHeight="true" AnchorHorizontal="100%" LabelWidth="120">
                        <Items>
                            <ext:FormPanel runat="server" ID="fp1" Frame="true" AnchorHorizontal="100%" Title="Thông tin" LabelWidth="120"
                                ...
                                <Items>
                                    ...
                                    <ext:Hidden runat="server" ID="hdfCapKhenThuongKyLuatHangLoat" />
                                    <ext:ComboBox runat="server" ID="cbbCapKhenThuongKyLuatHangLoat" DisplayField="Name" FieldLabel="Cấp khen thưởng - kỷ luật"
                                        ValueField="Id" AnchorHorizontal="99%" Editable="false" ItemSelector="div.list-item" StoreID="StoreCapKhenThuongKyLuat" TabIndex="6"
                                        LoadingText="Đang tải dữ liệu..." CtCls="requiredData">
                                        <Triggers>
                                            <ext:FieldTrigger Icon="Clear" HideTrigger="true" />
                                        </Triggers>
                                        ...
                                        <Listeners>
                                            <Expand Handler="StoreCapKhenThuongKyLuat.reload();" />
                                            <Select Handler="this.triggers[0].show(); hdfCapKhenThuongKyLuatHangLoat.setValue(this.getValue());" />
                                            <TriggerClick Handler="if (index == 0) { this.clearValue(); this.triggers[0].hide(); hdfCapKhenThuongKyLuatHangLoat.reset();}" />
                                        </Listeners>
                                    </ext:ComboBox>
                ...
                <Buttons>
                    ...
                    <ext:Button runat="server" ID="btnDongLaiHL" Text="Đóng lại" Icon="Decline">
                        <Listeners>
                            <Click Handler="wdKhenThuongKyLuatHangLoat.hide(); Ext.net.DirectMethods.ResetForm();" />
                        </Listeners>
                    </ext:Button>
                </Buttons>
            ...
            <ext:Window Modal="true" Hidden="true" runat="server" Layout="BorderLayout" ID="wdShowReport"
                ...
                <Listeners>
                    <BeforeShow Handler="pnReportPanel.remove(0); addHomePage(pnReportPanel, 'Homepage', '../../Report/ReportView.aspx?rp='+hdfTimeSheetHandlerTypeReport.getValue() + '&tungay=' + dfNgayBatDau.getRawValue() + '&denngay=' + dfNgayKetThuc.getRawValue(), hdfTitleReport.getValue());" />
                </Listeners>
                <Buttons>
