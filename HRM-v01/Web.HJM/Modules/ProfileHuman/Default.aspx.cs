...
namespace Web.HJM.Modules.ProfileHuman
{
    public partial class Default : BasePage
    {
        ...
        protected void filterRecruimentDateStore_Store_OnRefreshData(object sender, StoreRefreshDataEventArgs e)
        {
            try
            {
                DataTable table = DataHandler.GetInstance().ExecuteDataTable("SELECT DISTINCT YEAR(RecruimentDate) AS NGAYTUYENDUNG FROM hr_Record WHERE RecruimentDate IS NOT NULL ORDER BY NGAYTUYENDUNG ASC");
                List<object> obj = new List<object>();
                obj.Add(new { MA = -1, TEN = "Tất cả" });
                foreach (DataRow item in table.Rows)
                {
        ...
        protected void btnDelete_Click(object sender, DirectEventArgs e)
        {
            ...
            foreach (var item in selectedRows)
            {
                try
                {
                    ...
                    {
                        new RecordController().Delete(Convert.ToInt32(item.RecordID));
                        success++;
                    }
                }
                catch
        ...
        private void ExportToExcel(DataTable table)
        {
            try
            {
                var serverPath = Server.MapPath(relativePath + "/LyLichTrichNgach.xlsx");
                var workbook = new WorkBook();
                ...
                workbook.writeXLSX(serverPath);
                Response.AddHeader("Content-Disposition", "attachment; filename=" + "LyLichTrichNgach.xlsx");
                Response.WriteFile(serverPath);
                Response.End();
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            var hs = hr_RecordServices.GetById(Convert.ToInt32(hdfRecordId.Text));
                            hs.ImageUrl = path1;
                ...
                employeeDetail.SetProfileImage(path1);
                grp_HoSoNhanSu.Reload();
            }
            catch (Exception ex)
            {
        ...
        public void GetEmployeeDetailInformation()
        {
            if (!string.IsNullOrEmpty(hdfRecordId.Text))
            {
                ...
                var imageURL = "../../../File/ImagesEmployee/No_person.jpg";
