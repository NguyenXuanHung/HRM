...
{
    public partial class DismissalManagement : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!ExtNet.IsAjaxRequest)
            {
                ...
                if (btnEdit.Visible)
                {
                    gridDismissal.Listeners.RowDblClick.Handler +=
                        " if(CheckSelectedRows(gridDismissal)){btnUpdateAndSave.show();}";
                }
            }
        }
        ...
        protected void InsertOrUpdate(object sender, DirectEventArgs e)
        {
            ...
            //hide window
            wdDismissal.Hide();
            wdUpdateDismissal.Hide();

            //reload data
            gridDismissal.Reload();
            txtDescription.Text = "";
            RM.RegisterClientScriptBlock("resetform1", "ResetFormBusiness();");
