...
{
    public partial class SecondmentToManagement : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if(!ExtNet.IsAjaxRequest)
            {
                ...
                if(btnEdit.Visible)
                {
                    gridSecondmentTo.Listeners.RowDblClick.Handler +=
                        " if(CheckSelectedRows(gridSecondmentTo)){btnUpdateAndSave.show();}";
                }
            }
        }
        ...
        protected void InsertOrUpdate(object sender, DirectEventArgs e)
        {
            ...
            //hide window
            wdSecondmentTo.Hide();
            wdUpdateSecondmentTo.Hide();

            //reload data
            gridSecondmentTo.Reload();
            txtDescription.Text = "";
            RM.RegisterClientScriptBlock("resetform1", "ResetFormBusiness();");
        ...
        protected void EditMoveToClick(object sender, DirectEventArgs e)
        {
            ...
            if(!string.IsNullOrEmpty(business.FileScan))
            {
                ...
                if(pos != -1)
                {
                    string index = business.FileScan.Substring(pos + 1);
                    uploadFileScan.Text = index;
                }
        ...
        private void Update()
        {
            try
            {
                ...
                business.NewPosition = txtNewPosition.Text;
