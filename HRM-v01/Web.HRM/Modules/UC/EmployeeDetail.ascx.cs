...
namespace Web.HRM.Modules.UC
{
    public partial class EmployeeDetail : BaseUserControl
    {
        ...
        {
            hsImage.ImageUrl = @"~/" + Constant.PathLocationImageEmployee + @"/" + url;
        }
        ...
        {
            if (string.IsNullOrEmpty(hdfRecordId.Text)) return;
            StoregrpDiNuocNgoai.DataSource = GoAboardController.GetAll(null, null, Convert.ToInt32(hdfRecordId.Text), null, null, null, null, null); 
        ...
        {
            if (string.IsNullOrEmpty(hdfRecordId.Text)) return;
            StoreQuaTrinhDaoTao.DataSource = TrainingHistoryController.GetAll(null, null, Convert.ToInt32(hdfRecordId.Text), null, null, null, null, null, null);
        ...
        {
            if (string.IsNullOrEmpty(hdfRecordId.Text)) return;
            StoreKhenThuong.DataSource = RewardController.GetAll(null, null, Convert.ToInt32(hdfRecordId.Text), null, null, null, null, null, null);
        ...
        {
            if (string.IsNullOrEmpty(hdfRecordId.Text)) return;
            StoreKyLuat.DataSource = DisciplineController.GetAll(null, null, Convert.ToInt32(hdfRecordId.Text), null, null, null, null, null, null);
        ...
        protected void btnUpdateDaoTao_Click(object sender, DirectEventArgs e)
        {
            try
            {
                var trainingHistory = new TrainingHistoryModel();
                ...
                if (e.ExtraParams["Command"] == "Update")
                {
                    ...
                    trainingHistory.EditedBy = CurrentUser.User.UserName;
                    trainingHistory.Id = int.Parse(RowSelectionModelQuaTrinhDaoTao.SelectedRecordID);
                    //update
                    TrainingHistoryController.Update(trainingHistory);
                ...
                else
                {
                    ...
                    trainingHistory.EditedDate = DateTime.Now;
                    trainingHistory.EditedBy = CurrentUser.User.UserName;
                    trainingHistory.CreatedBy = CurrentUser.User.UserName;
                    //create
                    TrainingHistoryController.Create(trainingHistory);
                }
        ...
        protected void btnUpdateCongViec_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                if (!DatetimeHelper.IsNull(dfBHDenNgay.SelectedDate))
                    insurance.ToDate = dfBHDenNgay.SelectedDate;
                if (!DatetimeHelper.IsNull(dfBHTuNgay.SelectedDate))
                ...
                if (e.ExtraParams["Command"] == "Update")
                {
                    ...
                    InsuranceController.Update(insurance);
                    wdBaoHiem.Hide();
                }
                else
                {
                    InsuranceController.Insert(insurance);
                }
                if (e.ExtraParams["Close"] == "True")
                {
                    wdBaoHiem.Hide();
        ...
        protected void btnCapNhatDaiBieu_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                if (!DatetimeHelper.IsNull(dfDBTuNgay.SelectedDate))
                    dele.FromDate = dfDBTuNgay.SelectedDate;
                if (!DatetimeHelper.IsNull(dfDBDenNgay.SelectedDate))
        ...
        protected void btnUpdateDiNuocNgoai_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(hdfRecordId.Text))
                    model.RecordId = Convert.ToInt32(hdfRecordId.Text);
                if (!string.IsNullOrEmpty(hdfGoBoardNationId.Text))
                    model.NationId = Convert.ToInt32(hdfGoBoardNationId.Text);
                model.Note = txtGoAboardNote.Text;
                model.Reason = txtGoAboardReason.Text;
                model.CreatedDate = DateTime.Now;
                model.CreatedBy = CurrentUser.User.UserName;
                if (!DatetimeHelper.IsNull(dfNgayBatDau.SelectedDate))
                    model.StartDate = dfNgayBatDau.SelectedDate;
                if (!DatetimeHelper.IsNull(dfNgayKetThuc.SelectedDate))
                    model.EndDate = dfNgayKetThuc.SelectedDate;
                if (e.ExtraParams["Command"] == "Update")
                {
                    model.Id = Convert.ToInt32(RowSelectionModelDiNuocNgoai.SelectedRecordID);
        ...
        public string UploadFile(object sender, string relativePath)
        {
            ...
            var path = Path.Combine(Server.MapPath("~/"), fileRelativePath);
            if (File.Exists(path))
                return "";
           
            file.SaveAs(path);
            //File.Move(path, Server.MapPath(fileRelativePath));
        ...
        protected void btnUpdateHopDong_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                if (!DatetimeHelper.IsNull(dfHopDongNgayHopDong.SelectedDate))
                ...
                if (!DatetimeHelper.IsNull(dfHopDongNgayKiKet.SelectedDate))
                    contract.ContractEndDate = dfHopDongNgayKiKet.SelectedDate;
                if (!DatetimeHelper.IsNull(dfNgayCoHieuLuc.SelectedDate))
        ...
        protected void btnUpdateKhenThuong_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                var suffix = SystemConfigController.GetValueByNameFollowDepartment(SystemConfigParameter.SUFFIX_SOQDKHENTHUONG, departments);
                if (string.IsNullOrEmpty(suffix))
                    suffix = "QĐ";
                ...
                if (!DatetimeHelper.IsNull(dfKhenThuongNgayQuyetDinh.SelectedDate))
                ...
                if (e.ExtraParams["Command"] == "Update")
                {
                    ...
                    RewardController.Update(reward);
                    wdKhenThuong.Hide();
                }
                else
                {
                    RewardController.Create(reward);
                    if (e.ExtraParams["Close"] == "True")
                    {
                        wdKhenThuong.Hide();
                    }
        ...
        protected void btnCapNhatKyLuat_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                {
                    Dialog.Alert("Thông báo", "Không tìm thấy lý do kỷ luật. Vui lòng thử lại!");
                    return;
                }
                ...
                var departments = string.Join(",", CurrentUser.Departments.Select(d => d.Id));
                var suffix = SystemConfigController.GetValueByNameFollowDepartment(SystemConfigParameter.SUFFIX_SOQDKYLUAT, departments);
                if (string.IsNullOrEmpty(suffix))
                    suffix = "QĐ";
                ...
                if (!DatetimeHelper.IsNull(dfKyLuatNgayQuyetDinh.SelectedDate))
                ...
                {
                    DisciplineController.Create(discipline);
                    if (e.ExtraParams["Close"] == "True")
                    {
                        wdKyLuat.Hide();
                    }
        ...
        protected void btnUpdateQuanHeGiaDinh_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                {
                    if (!DatetimeHelper.IsNull(dfQHGDBatDauGiamTru.SelectedDate))
                        relationship.ReduceStartDate = dfQHGDBatDauGiamTru.SelectedDate;
                    if (!DatetimeHelper.IsNull(dfQHGDKetThucGiamTru.SelectedDate))
        ...
        protected void btnCapNhatQuaTrinhDieuChuyen_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                if (!DatetimeHelper.IsNull(dfQTDCNgayQuyetDinh.SelectedDate))
                    work.DecisionDate = dfQTDCNgayQuyetDinh.SelectedDate;
                if (!DatetimeHelper.IsNull(dfQTDCNgayCoHieuLuc.SelectedDate))
        ...
        protected void btnUpdateTaiSan_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                if (!DatetimeHelper.IsNull(tsDateField.SelectedDate))
        ...
        protected void Update_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                if (!DatetimeHelper.IsNull(dfKNLVTuNgay.SelectedDate))
                    workHistory.FromDate = dfKNLVTuNgay.SelectedDate;
                if (!DatetimeHelper.IsNull(dfKNLVDenNgay.SelectedDate))
        ...
        protected void btnUpdateChungChi_Click(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                if (!DatetimeHelper.IsNull(df_NgayCap.SelectedDate))
                    certify.IssueDate = df_NgayCap.SelectedDate;
                if (!DatetimeHelper.IsNull(df_NgayHetHan.SelectedDate))
        ...
        public void GetDataForBaoHiem()
        {
            ...
            else
            {
                ...
                if (!DatetimeHelper.IsNull(insurance.FromDate))
                    dfBHTuNgay.SetValue(insurance.FromDate);
                if (!DatetimeHelper.IsNull(insurance.ToDate))
        ...
        public void GetDataForDaiBieu()
        {
            ...
            else
            {
                ...
                if (!DatetimeHelper.IsNull(dele.FromDate))
                    dfDBTuNgay.SetValue(dele.FromDate);
                if (!DatetimeHelper.IsNull(dele.ToDate))
        ...
        public void GetDataForHopDong()
        {
            ...
            else
            {
                ...
                if (!DatetimeHelper.IsNull(contract.ContractEndDate))
                    dfHopDongNgayKiKet.SetValue(contract.ContractEndDate);
                if (!DatetimeHelper.IsNull(contract.ContractDate))
                    dfHopDongNgayHopDong.SetValue(contract.ContractDate);
                if (!DatetimeHelper.IsNull(contract.EffectiveDate))
        ...
        public void GetDataForKhenThuong()
        {
            ...
            else
            {
                ...
                if (!DatetimeHelper.IsNull(reward.DecisionDate))
                    dfKhenThuongNgayQuyetDinh.SetValue(reward.DecisionDate);
                cbHinhThucKhenThuong.Text = reward.FormName;
                hdfRewardFormId.Text = reward.FormRewardId.ToString();
                cbxCapKhenThuong.Text = reward.LevelName;
        ...
        public void GetDataForKyLuat()
        {
            ...
            else
            {
                ...
                cbHinhThucKyLuat.Text = discipline.FormName;
                hdfDisciplineFormId.Text = discipline.FormDisciplineId.ToString();
                cbxCapKyLuat.Text = discipline.LevelName;
        ...
        public void GetDataForTaiSan()
        {
            ...
            else
            {
                ...
                if (!DatetimeHelper.IsNull(asset.ReceiveDate))
        ...
        public void GetDataForChungChi()
        {
            try
            {
                ...
                else
                {
                    ...
                    if (!DatetimeHelper.IsNull(certificate.IssueDate))
                        df_NgayCap.SelectedDate = certificate.IssueDate.Value;
                    if (!DatetimeHelper.IsNull(certificate.ExpirationDate))
        ...
        public void GetDataForKinhNghiemLamViec()
        {
            ...
            else
            {
                ...
                if (work != null)
                {
                    ...
                    if (!DatetimeHelper.IsNull(work.FromDate))
                        dfKNLVTuNgay.SetValue(work.FromDate);
                    if (!DatetimeHelper.IsNull(work.ToDate))
        ...
        public void SetNgayHetHD()
        {
            ...
            if (contractTypeId == "" || DatetimeHelper.IsNull(dfNgayCoHieuLuc.SelectedDate)) return;
        ...
        private string GetFileName(string pathOfFile)
        {
            ...
            return pos != -1 ? pathOfFile.Substring(pos + 1) : "";
        }
        ...
        protected void btnAccept_Click(object sender, DirectEventArgs e)
        {
            ...
            try
            {
                hdfImage.Text = UploadFile(fufUploadControl, Constant.PathLocationImageEmployee);
                //update record
                if (!string.IsNullOrEmpty(hdfRecordId.Text))
                {
            ...
            hsImage.ImageUrl = @"~/" + Constant.PathLocationImageEmployee + @"/" + hdfImage.Text;
        }
    }
